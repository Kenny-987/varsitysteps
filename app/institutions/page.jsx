...
const Institutions = () => {
 ...
 // console.log(totalPages)
  ...
  useEffect(() => {
    const fetchInstitutes = async () => {
      try {
        ...
        const response = await fetch(`https://varsitysteps-server.onrender.com/institutes?page=${currentPage}&limit=${itemsPerPage}`)
  ...
  const search = async () => {
    if (query.length > 0) {
      try {
        const response = await fetch(`https://varsitysteps-server.onrender.com/institutes/search?query=${query}`)
        const data = await response.json()

...
const Institutions = () => {
 ...
 // console.log(totalPages)
  ...
  useEffect(() => {
    const fetchInstitutes = async () => {
      try {
        ...
        const response = await fetch(`https://early-flowers-shave.loca.lt/institutes?page=${currentPage}&limit=${itemsPerPage}`)
  ...
  const search = async () => {
    if (query.length > 0) {
      try {
        const response = await fetch(`https://early-flowers-shave.loca.lt/institutes/search?query=${query}`)
        const data = await response.json()

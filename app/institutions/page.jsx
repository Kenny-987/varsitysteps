...
const Institutions = () => {
 ...
 // console.log(totalPages)
  ...
  useEffect(() => {
    const fetchInstitutes = async () => {
      try {
        ...
        const response = await fetch(`http://10.1.10.89:3000/institutes?page=${currentPage}&limit=${itemsPerPage}`)
  ...
  const search = async () => {
    if (query.length > 0) {
      try {
        const response = await fetch(`http://10.1.10.89:3000/institutes/search?query=${query}`)
        const data = await response.json()

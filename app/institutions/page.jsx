...
const Institutions = () => {
 ...
 // console.log(totalPages)
  ...
  useEffect(() => {
    const fetchInstitutes = async () => {
      try {
        ...
        const response = await fetch(`http://https://varsitysteps-server.onrender.com:3000/institutes?page=${currentPage}&limit=${itemsPerPage}`)
  ...
  const search = async () => {
    if (query.length > 0) {
      try {
        const response = await fetch(`http://https://varsitysteps-server.onrender.com:3000/institutes/search?query=${query}`)
        const data = await response.json()

...
useEffect(()=>{
    const fetchTutor = async ()=>{
        // setLoading(true)
        try {
            const response  = await fetch(`https://early-flowers-shave.loca.lt/tutors/tutorprofile/${tutorID}`)
...
if(isAuthenticated){
    ...
    setLoading(true)
    try {
        const response = await fetch(`https://early-flowers-shave.loca.lt/api/connectionrequest`, {
            method: 'POST',
            headers: {
...
//function to check if user is connected
    const checkConnection = async()=>{
        try {
            const response = await fetch( 'https://early-flowers-shave.loca.lt/api/connectionstatus',{
                method:'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
...
//function to check for existing chats between tutor and student
    const checkChatStatus = async()=>{
        try {
           const response = await fetch(`https://early-flowers-shave.loca.lt/messages/checkchats?user1=${tutorID}&user2=${userData.id}`,{
               credentials:'include'
           })
           if(response){
               const data = await response.json()
...
//function to check if user has rated 
    const checkHasRated = async () => {
        try {
            const response = await fetch(`https://early-flowers-shave.loca.lt/tutors/checkrate/${tutorID}`,{
                credentials:'include'
            })
            if(response.ok){
                const data = await response.json()
...
const rateTutor = async (tutor_id,rater_id) => {
    try {
        const response = await fetch(`https://early-flowers-shave.loca.lt/tutors/rate`,{
            method:'POST',
            credentials:'include',
            headers: {
                'Content-Type': 'application/json',
